...
export default function CreativeEditor() {
  ...
  const [backgroundCandidateId, setBackgroundCandidateId] = useState<string | null>(null);
  const [cutoutCandidateId, setCutoutCandidateId] = useState<string | null>(null);
  const [backgroundCandidateDirection, setBackgroundCandidateDirection] = useState<"next" | "previous">("next");
  const [cutoutCandidateDirection, setCutoutCandidateDirection] = useState<"next" | "previous">("next");
    ...
    <main className={styles.workbench}>
      <header className={styles.topbar}>
        ...
        <div className={styles.topActions}>
          {status === "unconfigured" && <span className={`${styles.status} ${styles.statusLocked}`} role="status"><i className={styles.statusDot} />{providerStatusUnavailable ? "Live status unavailable" : providerSetupComplete ? "Unlock to render" : "Setup needed"}</span>}
          <Link className={styles.siteLink} href="/"><span>Storefront</span><span className={styles.siteLinkIcon} aria-hidden="true">↗</span></Link>
          <details className={styles.exportMenu}>
            <summary className={`${styles.orangeButton} ${styles.exportTrigger}`}>Export PNG <span aria-hidden="true">⌄</span></summary>
            <div className={styles.exportPopover} aria-label="Export options">
              <p className={styles.exportPopoverTitle}>Choose a format</p>
              <button className={styles.exportButton} type="button" onClick={() => void exportArtwork("card").catch(() => undefined)}>Portrait social card <span>1080 × 1350</span></button>
              <button className={styles.exportButton} type="button" onClick={() => void exportArtwork("banner").catch(() => undefined)}>Digital banner <span>1920 × 1080</span></button>
              {exportError && <div className={`${styles.notice} ${styles.noticeError}`} role="alert">{exportError}</div>}
      ...
      <div className={styles.layout}>
        <aside className={styles.rail} aria-label="Event details and source assets">
          <section className={styles.railSection}><div className={styles.sectionHeading}><p className={styles.eyebrow}>Event details</p><span className={styles.tinyLabel}>REV {String(project.revision).padStart(2, "0")}</span></div>
          ...
          <section className={styles.railSection}><p className={styles.eyebrow}>Format</p><div className={styles.formatSwitch}><button className={styles.formatButton} type="button" aria-pressed={project.format === "card"} onClick={() => switchLayout("card")}><strong>PORTRAIT</strong><span>1080 × 1350</span></button><button className={styles.formatButton} type="button" aria-pressed={project.format === "banner"} onClick={() => switchLayout("banner")}><strong>BANNER</strong><span>1920 × 1080</span></button></div></section>
          <section className={styles.railSection}><div className={styles.sectionHeading}><p className={styles.eyebrow}>Layer position</p><span className={styles.tinyLabel}>{project.format.toUpperCase()}</span></div>
        ...
        <section className={styles.main} aria-label="Creative proof canvas"><div className={styles.canvasBar}><p className={styles.eyebrow}>Proof</p><div className={styles.canvasMeta}><span>{project.format === "card" ? "1080 × 1350" : "1920 × 1080"}</span><span>PNG / sRGB</span></div></div>
          <div className={styles.proofStage}><i className={styles.corner} /><i className={styles.corner} /><div ref={proofFrame} className={`${styles.proofFrame} ${project.format === "banner" ? styles.banner : ""}`} tabIndex={0} aria-label="Creative proof editor" onKeyDown={handleCanvasKeyDown}><canvas ref={previewCanvas} className={styles.proofCanvas} aria-label="Rendered creative proof" />{selectedCandidate?.asset.source === "sample" && <span className={styles.sampleTag}>Previously generated sample</span>}<div className={styles.layerInteraction} role="application" aria-label="Click or drag a creative layer to edit" onPointerDown={handleCanvasPointerDown} onPointerMove={handleCanvasPointerMove} onPointerUp={finishCanvasDrag} onPointerCancel={(event) => finishCanvasDrag(event, true)}>{selectedLayer && selectedRect && <div className={styles.selectedOutline} aria-hidden="true" style={{ left: `${(selectedRect.x / canvasDimensions.width) * 100}%`, top: `${(selectedRect.y / canvasDimensions.height) * 100}%`, width: `${(selectedRect.width / canvasDimensions.width) * 100}%`, height: `${(selectedRect.height / canvasDimensions.height) * 100}%` }} />}{selectedLayer === "athlete" && selectedRect && (["nw", "ne", "sw", "se"] as const).map((corner) => <span key={corner} className={styles.resizeHandle} data-resize-corner={corner} aria-hidden="true" style={{ left: `${((corner.endsWith("w") ? selectedRect.x : selectedRect.x + selectedRect.width) / canvasDimensions.width) * 100}%`, top: `${((corner.startsWith("n") ? selectedRect.y : selectedRect.y + selectedRect.height) / canvasDimensions.height) * 100}%` }} />)}</div></div></div>
        </section>
        ...
        <aside className={`${styles.rail} ${styles.rightRail}`} aria-label="Background direction">
          ...
          <section className={styles.railSection}>
            ...
            {project.backgroundCandidates.length > 0 && (() => {
              const candidates = project.backgroundCandidates;
              const preferredId = backgroundCandidateId ?? candidates.find((item) => item.asset.id === project.assets.background?.id)?.id ?? candidates.find((item) => item.status === "ready")?.id ?? candidates[candidates.length - 1].id;
              const foundIndex = candidates.findIndex((item) => item.id === preferredId);
              const index = foundIndex < 0 ? candidates.length - 1 : foundIndex;
              const candidate = candidates[index];
              const applied = candidate.asset.id === project.assets.background?.id;
              const pending = candidate.status === "pending";
              const candidateUrl = assetUrls[candidate.asset.blobKey] ?? assetUrls[candidate.asset.id] ?? candidate.asset.url;
              const adjacentCard = (adjacentIndex: number, direction: "previous" | "next") => {
                const adjacent = candidates[adjacentIndex];
                if (!adjacent) return <span className={styles.candidatePeekSpacer} aria-hidden="true" />;
                const url = assetUrls[adjacent.asset.blobKey] ?? assetUrls[adjacent.asset.id] ?? adjacent.asset.url;
                return <button className={styles.candidatePeek} type="button" aria-label={`Show ${direction} background candidate: ${adjacent.asset.name}`} title={adjacent.asset.name} onClick={() => { setBackgroundCandidateDirection(direction); setBackgroundCandidateId(adjacent.id); }}>
                  {url ? <img src={url} alt="" /> : <span className={styles.candidatePeekFallback} aria-hidden="true" />}
                </button>;
              };
              return <div className={styles.candidateCarousel} role="group" aria-label="Background candidates">
                <span className={styles.carouselCount} aria-live="polite">{index + 1} of {candidates.length}</span>
                <div className={`${styles.candidateDeck} ${candidates.length === 1 ? styles.candidateDeckSingle : ""}`}>
                  {candidates.length > 1 && adjacentCard(index - 1, "previous")}
                  <article className={`${styles.candidate} ${applied ? styles.selected : ""} ${backgroundCandidateDirection === "previous" ? styles.carouselPrevious : styles.carouselNext}`} key={candidate.id}>
                    <div className={`${styles.candidateVisual} ${index % 3 === 1 ? styles.alt : index % 3 === 2 ? styles.revision : ""}`}>
                      {candidateUrl && <img className={styles.candidateImage} src={candidateUrl} alt="" />}
                      <span>{pending ? "Rendering…" : candidate.status === "failed" ? "Render failed" : candidate.asset.source === "sample" ? "Previously generated sample" : candidate.status === "ready" ? "Ready to review" : "Unavailable"}</span>
                    </div>
                    <div className={styles.candidateInfo}><strong>{candidate.asset.name}</strong><span>{pending ? "PENDING" : candidate.asset.source === "sample" ? "LOCAL" : candidate.status === "ready" ? "NEW" : "FAILED"}</span></div>
                    {candidate.status === "failed" && <p className={styles.candidateWarning}>{candidate.warning || "The render failed. Try again later."}</p>}
                    {candidate.status === "ready" && <button className={styles.candidateAction} type="button" onClick={() => { setBackgroundCandidateId(candidate.id); applyCandidate(candidate.id); }}>{applied ? "Applied to proof" : "Apply to proof"}</button>}
                  </article>
                  {candidates.length > 1 && adjacentCard(index + 1, "next")}
                </div>
              </div>;
            })()}
          </section>
          {statusMessage && <div className={styles.rightRailStatus} role="status"><i className={styles.recoveryDot} aria-hidden="true" /><span>{statusMessage}</span></div>}
        </aside>
      </div>
    </main>
  );
